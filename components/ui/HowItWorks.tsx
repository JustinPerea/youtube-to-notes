  ...
  return (
    <section className="how-it-works my-25">
      ...
      <div className="steps grid grid-cols-1 md:grid-cols-3 gap-8 relative">
        ...
        <div className="hidden md:block absolute top-12 left-1/4 right-1/4 h-0.5 bg-gradient-to-r from-[var(--card-border)] via-[var(--accent-pink-soft)] to-[var(--card-border)] z-0"></div>
        
        {steps.map((step, index) => (
          <div key={index} className="step text-center relative z-10">
            <div className="step-number w-24 h-24 bg-[var(--card-bg)] border-2 border-[var(--card-border)] rounded-full flex items-center justify-center text-4xl mx-auto mb-6 relative transition-all duration-300 backdrop-blur-[20px] hover:transform hover:scale-110 hover:border-[var(--accent-pink)] hover:shadow-[0_8px_24px_rgba(255,107,157,0.2)] aspect-square">
              {step.number}
            </div>
